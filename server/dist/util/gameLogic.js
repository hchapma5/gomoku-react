...
exports.checkDraw = exports.checkWin = exports.buildGameBoard = void 0;
const constants_1 = require("../constants");
function buildGameBoard(size, movelist) {
    const stones = [...Array(size)].map(() => Array(size).fill(constants_1.Stone.EMPTY));
    movelist.forEach((move) => {
        stones[move.row][move.col] = move.player;
    });
    return stones;
}
exports.buildGameBoard = buildGameBoard;
function checkHorizontalWin(player, stones) {
...
function checkVerticalWin(player, stones) {
    for (let i = 0; i < stones.length; i++) {
        let count = 0;
        for (let j = 0; j < stones[i].length; j++) {
...
function checkDiagonalWin(player, stones) {
...
function checkWin(player, stones) {
    return (checkHorizontalWin(player, stones) ||
        checkVerticalWin(player, stones) ||
        checkDiagonalWin(player, stones));
}
exports.checkWin = checkWin;
function checkDraw(stones) {
    return stones.every((row) => row.every((tile) => tile !== constants_1.Stone.EMPTY));
}
